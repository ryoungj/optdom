...
if __name__ == "__main__":
    ...
    start_step = 0
    algorithm_dict = None
    only_eval_last = (args.dataset == "DomainNet")
    ...
    for step in range(start_step, n_steps):
        ...
        if (step % checkpoint_freq == 0) or (step == n_steps - 1):
            ...
            if (not only_eval_last) or (step == n_steps - 1):
                ...
                results.update(
                    {
                        'in_acc_all': average_by_filtering(results, lambda k: 'in_acc' in k),
                        ...
                        'in_acc_tgt': average_by_filtering(results,
                                                           lambda k: k in [f'env{env_i}_in_acc' for env_i in
                                                                           args.test_envs]),
                        'out_acc_all': average_by_filtering(results, lambda k: 'out_acc' in k),
