...
if __name__ == "__main__":
    ...
    parser.add_argument('--top_k', type=int, default=0, required=True)
    ...
    if args.task == "unsupervised_domain_generalization":
        assert args.test_env == -1
        records = reporting.get_grouped_records(records, group_test_envs=False)
        records = records.filter(
            lambda r:
            r['dataset'] == args.dataset and
            r['algorithm'] == args.algorithm
        )
        SELECTION_METHODS = [model_selection.ValidAllLossSelectionMethod]
    else:
    ...
    for selection_method in SELECTION_METHODS:
        ...
        for group in records:
            ...
            for i, (run_acc, hparam_records) in enumerate(best_hparams):
                if args.top_k > 0 and i == args.top_k:
                    break
                print(f"\t{run_acc}")
