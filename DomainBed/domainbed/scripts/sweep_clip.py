...
def make_args_list(n_trials, dataset_names, algorithms, n_hparams_from, n_hparams, steps,
    data_dir, task, holdout_fraction, single_test_envs, wandb_proj, wandb_group,
    only_eval, always_rerun, warmstart_model_ckpt, hparams):
    args_list = []
...
if __name__ == "__main__":
    ...
    parser.add_argument('command', choices=['launch', 'delete_incomplete', 'delete_useless', 'delete_and_launch'])
    ...
    parser.add_argument('--train_script', type=str, default='domainbed.scripts.train')
    ...
    args_list = make_args_list(
        ...
        warmstart_model_ckpt=args.warmstart_model_ckpt,
        hparams=args.hparams
    )

    jobs = [Job(train_args, args.output_dir, args.train_script) for train_args in args_list]

    # for job in jobs:
    #     print(job)
    print("Sweeping setup:")
    print("\tDataset: {}, algorithm: {}, task: {}.\n\tn_params: {}, n_trials: {}, hparams: {}\n".format(
        args.datasets, args.algorithms, args.task, args.n_hparams, args.n_trials, args.hparams
    ...
    # print([j.train_args for j in jobs if j.state == Job.INCOMPLETE])
    ...
    elif args.command == 'delete_useless':
        all_output_dirs = [job.output_dir for job in jobs]
        all_cur_dirs = [os.path.join(args.output_dir, o) for o in os.listdir(args.output_dir)
                        if os.path.isdir(os.path.join(args.output_dir, o))]
        print('Deleting...')
        num = 0
        for dir in all_cur_dirs:
            if dir not in all_output_dirs:
                shutil.rmtree(dir)
                num += 1
        print(f'Deleted {num} directories!')
