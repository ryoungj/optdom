...
class SelectionMethod:
    ...
    @classmethod
    def sweep_acc(self, records, return_extra=False):
        """
        Given all records from a single (dataset, algorithm, test env) pair,
        ...
        if len(_hparams_accs):
            if return_extra and 'ext_acc' in _hparams_accs[0][0]:
                return (_hparams_accs[0][0]['test_acc'], *_hparams_accs[0][0]['ext_acc'])
            else:
    ...
    def best_record(self, records):
        ...
        _hparams_accs = self.hparams_accs(records)
        if len(_hparams_accs):
            assert len(_hparams_accs[0][1]) == 1
            return _hparams_accs[0][1][0]
        else:
            return None
...
class OracleSelectionMethod(SelectionMethod):
    ...
    def run_acc(self, run_records):
        ...
        test_in_acc_key = 'in_acc_tgt'
        test_out_acc_key = 'out_acc_tgt'
        ext_acc_key = 'out_acc_src'
        ...
        return {
            ...
            'ext_acc': (chosen_record[ext_acc_key], chosen_record[test_in_acc_key]),
        }
...
class IIDAccuracySelectionMethod(SelectionMethod):
    ...
    def _step_acc(self, record):
        ...
        return {
            ...
            'ext_acc': (record[ext_acc_key], record[test_in_acc_key]),
        }
