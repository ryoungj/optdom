...
from domainbed.utils import PLLogisticRegression
...
def build_clip_imagenet_model(ckpt_path):
    ...
    model = model_class(feature_dim, num_classes,  # th inout shame becomes the output dim
                        orig_num_domains, hparams, pretrained, idx2class)

    state_dict = checkpoint["model_dict"]
    if 'classifier_head.weight' in state_dict:  # after we change `num_classes`, the shape may be incompatible
        del state_dict['classifier_head.weight']
        del state_dict['classifier_head.bias']

    if 'classifier.linear.weight' in state_dict:
        model.classifier = PLLogisticRegression(input_dim=feature_dim,
                                                num_classes=num_classes)

    missing_keys, unexpected_keys = model.load_state_dict(state_dict, strict=False)
    print("Missing: {}. Unexpected: {}".format(missing_keys, unexpected_keys))

    # no precomputation anyway
    if isinstance(model, algorithms.CLIPPretrained):
        model.transform = torch.nn.Identity()  # just for compatibility
